...
platforms = {
    "windows-x64": Platform(pypi_suffix="win_amd64", blender_tag="windows-x64"),
    "linux-x64": Platform(pypi_suffix="manylinux2014_x86_64", blender_tag="linux-x64"),
    "macos-arm64": Platform(pypi_suffix="macosx_12_0_arm64", blender_tag="macos-arm64"),
    "macos-x64": Platform(pypi_suffix="macosx_10_16_x86_64", blender_tag="macos-x64"),
}

packages_to_remove = {"imagecodecs", "numpy"}
...
def build_extension(platform: Platform, python_version="3.11") -> None:
    ...
    for f in wheel_dirpath.glob("*.whl"):
        if any([package in f.name for package in packages_to_remove]):
            ...
            platform.blender_tag == "macos-arm64"
            and "lxml" in f.name
    ...
    manifest["wheels"] = [f"./wheels/{f.name}" for f in wheel_dirpath.glob("*.whl")]
